package com.tb.demo.utils.hangview.custom;
...
public class RippleView extends RelativeLayout {
    ...
    private void init(final Context context, final AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            hasToZoom = typedArray.getBoolean(R.styleable.RippleView_rv_zoom,
                    false);
            isCentered = typedArray.getBoolean(
                    R.styleable.RippleView_rv_centered, false);
            DURATION = typedArray.getInteger(
                    R.styleable.RippleView_rv_rippleDuration, DURATION);
            FRAME_RATE = typedArray.getInteger(
                    R.styleable.RippleView_rv_framerate, FRAME_RATE);
            PAINT_ALPHA = typedArray.getInteger(
                    R.styleable.RippleView_rv_alpha, PAINT_ALPHA);
            ripplePadding = typedArray.getDimensionPixelSize(
                    R.styleable.RippleView_rv_ripplePadding, 0);
            canvasHandler = new Handler();
            zoomScale = typedArray.getFloat(
                    R.styleable.RippleView_rv_zoomScale, 1.03f);
            zoomDuration = typedArray.getInt(
                    R.styleable.RippleView_rv_zoomDuration, 100);
            typedArray.recycle();
        } else {
            rippleColor = getResources().getColor(
                    R.color.common_title_background);
            rippleType = 0;
            hasToZoom = false;
            isCentered = false;
            ripplePadding = 0;
